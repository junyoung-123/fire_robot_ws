...
## 최신 코드와 자료 (2026-09-26)

**최신 센서 기반 단일문 개방 브랜치: `codex/observed-door-angle-20260922`.** 이전 주행·접촉 성공 브랜치와 자료는 보존합니다. 아래 과거 절의 '최신'은 그 날짜 기준입니다.

- [Claude용 이미지·영상 인계](docs/handoff/2026-09-26/00_START_HERE.md) / [주제·슬라이드별 자료 ID](docs/handoff/2026-09-26/03_TOPIC_AND_SLIDE_MAP.md) / [검색 가능한 원본 색인](docs/handoff/2026-09-26/01_ASSET_INDEX.csv)
- [최신 코드 실행 범위와 확인 결과](docs/RELEASE_20260926.md) / [관측 입력과 R34 실험](ROBOT_OBSERVATION_ONLY.md) / [실물 센서 인터페이스](REAL_SENSOR_INTERFACE.md)
- R34 단일문: 손잡이 YOLO/RGB-D, 로봇 엔코더 기반 레버 추정, RGB-D/2D LiDAR 문각 관측으로 개방·재확인·해제·복귀 PASS. 문/레버 정답값은 독립 평가에서만 사용합니다. 관측 개방각 117.776도, 평가값 117.839도입니다.
- 월드 1~5 주행은 09-16 저장 지도 + sim_odom + 단순 힌지 backend 기록입니다. 최신 R34를 다섯 월드에 통합한 FULL 또는 실물 성공을 뜻하지 않습니다.
- 09-26 확인: Python 138개 문법 검사, 회귀 테스트 68개 PASS. 제어 소스는 R34 실행 snapshot과 일치합니다. 새 Gazebo 실행은 하지 않았습니다.
- 이미지 56개·영상 6개를 날짜/버전/용도/주의사항으로 분류했습니다. 오탐·대기 화면·이전 도표는 `archive/`에 남겼습니다. 손잡이 E1의 P/R 100%는 기존 예측의 Gazebo 레버 양성 100장 부분집합이며 실제 사진 성능이 아닙니다.

**실행 주의:** 현재 로봇 센서 전용 경로는 `scripts/start_robot_observation.sh`입니다. 과거 비교용 launch 기본값을 그대로 쓰면 문 관절 피드백 경로가 선택될 수 있습니다. 복제 환경에서는 스크립트의 `/home/junyoung/...` 경로와 ROS 설치를 먼저 확인하세요. 정책·안전 파라미터는 유지되며 '설정값/하드코딩 0'을 주장하지 않습니다.

## 이전 코드와 자료 (2026-09-17 보존)
